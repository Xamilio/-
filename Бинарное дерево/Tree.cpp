...
void Tree::Insert(Node* newNode)
{
    ...
    while (node != nullptr)
    {
        ...
        if (strcmp(newNode->LastName, node->LastName) < 0)
        {
            node = node->m_left;
    ...
    else if (strcmp(newNode->LastName, temp->LastName) < 0)
    {
        temp->m_left = newNode;
    }
    else
...
void Tree::Print(const Node* node) const
{
    if (node != nullptr)
    {
        ...
        cout << node->LastName << '\t' << node->Name <<  '\t' << node->SurName << '\t' << node->Grade << endl;
...
void Tree::Remove(Node* removeNode)
{
    if (removeNode != nullptr)
    {
        ...
        if (temp != removeNode)
        {
            ...
            strcpy_s(removeNode->SurName, 15U, temp->SurName);
            removeNode->Grade = temp->Grade;
        }
...
Node* Tree::Search(Node* node, const char* key) const
{
    ...
    while (node != nullptr && strcmp(key, node->LastName) != 0)
    {
        if (strcmp(key, node->LastName) < 0)
        {
            node = node->m_left;
        }
        else
